...
func TestParseHeaderRejectsBad(t *testing.T) {
	...
	if !strings.Contains(string(raw), `"version": 1`) {
		t.Fatalf("expected version 1 header, got:\n%s", raw)
	}

	if _, err := ParseHeader([]byte(`{"version":99,"master":"AA==","slots":[{"public_key":"x"}],"auth":"AA=="}`)); err == nil {
		t.Error("want error for an unsupported version")
	}
	if _, err := ParseHeader([]byte(`{"version":1,"master":"AA==","slots":[],"auth":"AA=="}`)); err == nil {
		t.Error("want error for empty slots")
	}
	if _, err := ParseHeader([]byte(`{"version":1,"slots":[{"public_key":"x"}],"auth":"AA=="}`)); err == nil {
		t.Error("want error for missing master")
	}
	if _, err := ParseHeader([]byte(`{"version":1,"master":"AA==","slots":[{"public_key":"x"}]}`)); err == nil {
