...
func TestParseHeaderRejectsBad(t *testing.T) {
	...
	raw, _ := good.Marshal()
	if !strings.Contains(string(raw), `"version": 6`) {
		t.Fatalf("expected version 6 header, got:\n%s", raw)
	}
	...
	for _, old := range []string{"1", "2", "3", "4", "5"} {
	...
	if _, err := ParseHeader([]byte(`{"version":6,"sign_pub":"ab","master":"AA==","slots":[{"public_key":"x"}],"auth":"AA=="}`)); err == nil {
		t.Error("want error for missing vault id")
	}
	if _, err := ParseHeader([]byte(`{"version":6,"vault_id":"v1","master":"AA==","slots":[{"public_key":"x"}],"auth":"AA=="}`)); err == nil {
		t.Error("want error for missing signing public key")
	}
	if _, err := ParseHeader([]byte(`{"version":6,` + idAndKey + `"master":"AA==","slots":[],"auth":"AA=="}`)); err == nil {
		t.Error("want error for empty slots")
	}
	if _, err := ParseHeader([]byte(`{"version":6,` + idAndKey + `"slots":[{"public_key":"x"}],"auth":"AA=="}`)); err == nil {
		t.Error("want error for missing master")
	}
	if _, err := ParseHeader([]byte(`{"version":6,` + idAndKey + `"master":"AA==","slots":[{"public_key":"x"}]}`)); err == nil {
