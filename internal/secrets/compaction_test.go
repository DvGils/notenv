...
// written and verified but before the folded segments are removed. No write is
// lost, and a clean re-run finishes the job.
...
// TestCompactKeepsConcurrentWrite injects a write from another machine right
// after the compaction has listed its objects. The new segment was never
// listed, so it is not deleted and survives the fold.
...
// TestConcurrentCompactionCoexistsThenCollapses runs a second compaction in the
// instant before the first writes its snapshot. Both snapshots survive (neither
// listed the other), the fold reads both correctly, and a later compaction
// collapses them into one.
func TestConcurrentCompactionCoexistsThenCollapses(t *testing.T) {
	ctx := context.Background()
	store, mk := newStoreMaster(t)
	a := newFixture(t, store, mk, "m1")
	s := a.set(a.fold(), "A", "1")
	...
	if snaps, segs := classify(t, store); snaps != 2 || segs != 0 {
		t.Fatalf("want 2 coexisting snapshots and 0 segments, got %d/%d", snaps, segs)
	}
	if state := a.fold(); state.Secrets["A"] != "1" || state.Secrets["B"] != "2" {
	...
	if snaps, segs := classify(t, store); snaps != 1 || segs != 0 {
		t.Fatalf("compaction should collapse to one snapshot, got %d/%d", snaps, segs)
	}
}

// TestCompactVanishedSegmentFailsCleanly has another machine fully compact in
// the window between this compaction's listing and its reads, so the listed
// segments vanish. The compaction fails, but no data is lost.
func TestCompactVanishedSegmentFailsCleanly(t *testing.T) {
	ctx := context.Background()
	store, mk := newStoreMaster(t)
	a := newFixture(t, store, mk, "m1")
	s := a.set(a.fold(), "A", "1")
...
func TestFoldRejectsCorruptSegment(t *testing.T) {
	...
	if err := store.Put(ctx, "proj/seg-bad-deadbeef.age", []byte("not age ciphertext")); err != nil {
		t.Fatal(err)
	}
